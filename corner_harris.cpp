...
void Harris_Demo(int, void*)
{
	...
	normalize(dst, norm_dst, 0, 255, NORM_MINMAX, CV_32FC1, Mat());   //像素可能会很大，归一化到0-255
	imshow("norm_dst", norm_dst);
	convertScaleAbs(norm_dst, normScaleDst);  //图像增强,将归一化后的线性变换成8位无符号整型，将白色看不见，变成黑色可看见。
	imshow("normScaleDst", normScaleDst);
	...
	imwrite("norm_dst.jpg", norm_dst);
	imwrite("normScaleDst.jpg", normScaleDst);
}